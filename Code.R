...
## Error might be produced when one of the imputation failed but it won't affect the functionality of the code since only completed imputation result is used.
for (z in 1:20) {
  ...
  input3_t <- input3 %>% select(1,2,3,5,6,13:18)
  input3_t <- input3_t %>% filter(!is.na(small))
  ### This code might prodece error, but it is fine
  ...
  l_estimate_sel <- left_join(l_estimate_sel,pop, by=c("year"="year","o_name"="i_iso3"))
...
res_od=l_estimate %>% 
  ...
  #`na.rm = T` should not be necessary in real data 
  summarise(q50=median(value,na.rm = T),
            q10=quantile(value,0.1,na.rm = T),
            q90=quantile(value,0.9,na.rm = T))
write_csv(res_od,file=paste0("outputs ",Sys.Date(), "/res_od.csv"))
